    ...
    private fun TraversalContext.intOverflowCheck(op: BinopExpr, leftRaw: PrimitiveValue, rightRaw: PrimitiveValue) {
        ...
        val overflow = when (op) {
            ...
            is JMulExpr -> when (sort.type) {
                is ByteType -> lowerIntMulOverflowCheck(left, right, kotlin.Byte.MIN_VALUE.toInt(), kotlin.Byte.MAX_VALUE.toInt())
                is ShortType -> lowerIntMulOverflowCheck(left, right, kotlin.Short.MIN_VALUE.toInt(), kotlin.Short.MAX_VALUE.toInt())
                is IntType -> higherIntMulOverflowCheck(left, right, kotlin.Int.SIZE_BITS, kotlin.Int.MIN_VALUE.toLong()) { it: UtExpression -> it.toIntValue() }
                is LongType -> higherIntMulOverflowCheck(left, right, kotlin.Long.SIZE_BITS, kotlin.Long.MIN_VALUE) { it: UtExpression -> it.toLongValue() }
                else -> null
            }
            else -> null
        }
